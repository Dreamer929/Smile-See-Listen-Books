...
- (NSArray *)getDataWithCatefgory:(NSInteger)category{
    
    NSArray * paths = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES);
    NSString * documentPath = [paths objectAtIndex:0];
    NSString * dbPath = [documentPath stringByAppendingPathComponent:@"poem.sqlite"];
    NSFileManager * fm = [NSFileManager defaultManager];
    BOOL isExist = [fm fileExistsAtPath:dbPath];
    if (!isExist) {
        NSString * backDbPath = [[NSBundle mainBundle] pathForResource:@"poem" ofType:@"sqlite"];
        [fm copyItemAtPath:backDbPath toPath:dbPath error:nil];
    }
    ...
    NSMutableArray * allPoems = [NSMutableArray array];
    NSString * sql = @"SELECT * FROM poetries WHERE category=?";//?必须为对象
    FMResultSet * resultSet = [self.db executeQuery:sql,[NSNumber numberWithInteger:category]];
